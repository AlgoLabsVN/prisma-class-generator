...
import { PrismaCreateDtoGenerator } from '../generator'
...
export class FileComponent implements Echoable {
	...
	constructor(input: { classComponent: ClassComponent; output: string }) {
		...
		this.filename = `${kebabCase(
			classComponent.name.replace('Dto', ''),
		)}.dto.ts`
		this.resolveImports()
	}
	...
	resolveImports() {
		const generator = PrismaCreateDtoGenerator.getInstance()
