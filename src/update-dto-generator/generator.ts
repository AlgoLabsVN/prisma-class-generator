...
export const PrismaUpdateDtoGeneratorOptions = {
	makeIndexFile: {
...
export type PrismaUpdateDtoGeneratorOptionsKeys =
	keyof typeof PrismaUpdateDtoGeneratorOptions
export type PrismaUpdateDtoGeneratorConfig = Partial<
	Record<PrismaUpdateDtoGeneratorOptionsKeys, any>
>

export class PrismaUpdateDtoGenerator {
	...
	static getInstance(options?: GeneratorOptions) {
		if (PrismaUpdateDtoGenerator.instance) {
			return PrismaUpdateDtoGenerator.instance
		}
		PrismaUpdateDtoGenerator.instance = new PrismaUpdateDtoGenerator(
			options,
		)
		return PrismaUpdateDtoGenerator.instance
	}
	...
	run = async (): Promise<void> => {
		...
		files.forEach((fileRow) => {
			...
			fileRow.imports.push(
				new ImportComponent('@nestjs/swagger', 'PartialType'),
			)
	...
	getConfig = (): PrismaUpdateDtoGeneratorConfig => {
		...
		const result: PrismaUpdateDtoGeneratorConfig = {}
		for (const optionName in PrismaUpdateDtoGeneratorOptions) {
			const { defaultValue } = PrismaUpdateDtoGeneratorOptions[optionName]
